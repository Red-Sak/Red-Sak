          ...
          (function () {
            ...
            $(".blocsapp-special-menu blocsnav li").each(function () {
              $(this).parent().hasClass("dropdown-menu")
                ...
                : ((k += a),
                  $(this)
                    .attr("style", "animation-delay:" + k + "ms")
                    .addClass("animated " + j));
            });
          })(),
          setTimeout(function () {
...
function setUpDropdownSubs() {
  $("ul.dropdown-menu [data-toggle=dropdown]").on("click", function (a) {
    ...
    b.offset().left + b.width() > $(window).width() &&
      b.addClass("dropmenu-flow-right");
  });
}
...
function inViewCheck() {
  $($(".hideMe").get().reverse()).each(function (d) {
    ...
    if ((f.height() > $(window).height() && (c = f.offset().top), c < b)) {
      ...
      f.css("visibility", "hidden").removeAttr("class"),
        setTimeout(function () {
...
function setUpLightBox() {
  window.targetLightbox,
    $(document)
      .on("click", "[data-lightbox]", function (f) {
        ...
        targetLightbox.attr("data-gallery-id") &&
          (j = targetLightbox.attr("data-gallery-id")),
        ...
        $("body").append(g),
          ...
          "no-gallery-set" == j
            ...
            : (0 == $('a[data-gallery-id="' + j + '"]').index(targetLightbox) &&
                $(".prev-lightbox").hide(),
              $('a[data-gallery-id="' + j + '"]').index(targetLightbox) ==
                $('a[data-gallery-id="' + j + '"]').length - 1 &&
                $(".next-lightbox").hide()),
    ...
    $(document).on("click", ".next-lightbox, .prev-lightbox", function (f) {
      ...
      targetLightbox.attr("data-gallery-id") &&
        ((g = targetLightbox.attr("data-gallery-id")),
        (d = $('a[data-gallery-id="' + g + '"]').index(targetLightbox)),
        (c = $('a[data-gallery-id="' + g + '"]').eq(d + 1))),
        $(this).hasClass("prev-lightbox") &&
          ((c = $('a[data-gallery-id="' + g + '"]').eq(d - 1)),
          "no-gallery-set" == g && (c = $("a[data-lightbox]").eq(d - 1)));
      var h = c.attr("data-lightbox");
      if (".mp4" == h.substring(h.length - 4)) {
        var b = "";
        1 == c.attr("data-autoplay") && (b = "autoplay"),
          $("#lightbox-image, .lightbox-caption").hide(),
        ...
        $("#lightbox-image").attr("src", h).show(),
          $(".lightbox-caption").html(c.attr("data-caption")).show(),
          $("#lightbox-video-container").hide();
      }
      (targetLightbox = c),
        $(".next-lightbox, .prev-lightbox").hide(),
        "no-gallery-set" == g
          ? ($("a[data-lightbox]").index(c) !=
              $("a[data-lightbox]").length - 1 && $(".next-lightbox").show(),
            $("a[data-lightbox]").index(c) > 0 && $(".prev-lightbox").show())
          : ($('a[data-gallery-id="' + g + '"]').index(c) !=
              $('a[data-gallery-id="' + g + '"]').length - 1 &&
              $(".next-lightbox").show(),
            $('a[data-gallery-id="' + g + '"]').index(c) > 0 &&
              $(".prev-lightbox").show());
    });
}
