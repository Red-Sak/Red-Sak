  ...
  var f = {
    ...
    methods: {
      init: function (i) {
        ...
        e(h).bind("submit", function (n) {
          ...
          m.trigger("submit.validation").trigger(
          ...
          if (o) {
            ...
            if (e.isFunction(j.options.submitError)) {
              j.options.submitError(
                l,
                n,
                m.jqBootstrapValidation("collectErrors", true)
              );
            }
          } else {
            l.removeClass("error");
        ...
        return this.each(function () {
          ...
          if (j.options.sniffHtml) {
            ...
            if (
              q.attr("max") !== undefined ||
              q.attr("aria-valuemax") !== undefined
            ) {
              var s =
                q.attr("max") !== undefined
                  ? q.attr("max")
                  : q.attr("aria-valuemax");
              u =
                "Too high: Maximum of '" +
                s +
            ...
            if (
              q.attr("min") !== undefined ||
              q.attr("aria-valuemin") !== undefined
            ) {
              var n =
                q.attr("min") !== undefined
                  ? q.attr("min")
                  : q.attr("aria-valuemin");
              u =
                "Too low: Minimum of '" +
                n +
            ...
            if (
              q.attr("required") !== undefined ||
              q.attr("aria-required") !== undefined
            ) {
            ...
            if (
              q.attr("type") !== undefined &&
              q.attr("type").toLowerCase() === "number"
            ) {
          ...
          do {
            ...
            e.each(o, function (x, y) {
              if (q.data("validation" + y + "Shortcut") !== undefined) {
                e.each(
                  q.data("validation" + y + "Shortcut").split(","),
                  function (A, z) {
                    r.push(z);
                  }
                );
          ...
          e.each(l, function (x, z) {
            ...
            e.each(j.validatorTypes, function (D, E) {
              ...
              if (!y && q.data("validation" + z + d(E.name)) !== undefined) {
                m[D].push(
                  e.extend(true, { name: d(E.name), message: A }, E.init(q, z))
                );
                y = true;
              }
            });
          ...
          t.data(
            "original-contents",
            t.data("original-contents") ? t.data("original-contents") : t.html()
          );
          t.data(
            "original-role",
            t.data("original-role") ? t.data("original-role") : t.attr("role")
          );
          p.data(
            "original-classes",
            p.data("original-clases")
              ? p.data("original-classes")
              : p.attr("class")
          );
          q.data(
            "original-aria-invalid",
            q.data("original-aria-invalid")
              ? q.data("original-aria-invalid")
              : q.attr("aria-invalid")
          );
          q.bind("validation.validation", function (x, z) {
            ...
            e.each(m, function (A, B) {
              if (
                ...
                y.length ||
                (z && z.includeEmpty) ||
                (!!j.validatorTypes[A].blockSubmit && z && !!z.submitting)
              ) {
          ...
          q.bind("submit.validation", function () {
            return q.triggerHandler("change.validation", { submitting: true });
          });
          q.bind(
            [
              "keyup",
            ...
            function (y, z) {
              ...
              p.find("input,textarea,select").each(function (C, D) {
                var A = w.length;
                e.each(
                  e(D).triggerHandler("validation.validation", z),
                  function (E, F) {
                    w.push(F);
                  }
                );
              ...
              if (w.length) {
                ...
                if (j.options.semanticallyStrict && w.length === 1) {
                  t.html(
                    w[0] +
                      (j.options.prependExistingHelpBlock
                        ? t.data("original-contents")
                        : "")
                  );
                } else {
                  t.html(
      ...
      hasErrors: function () {
        ...
        this.each(function (j, k) {
          h = h.concat(
            e(k).triggerHandler("getValidators.validation")
              ? e(k).triggerHandler("validation.validation", {
                  submitting: true,
                })
              : []
          );
    ...
    validatorTypes: {
      ...
      regex: {
        name: "regex",
        init: function (i, h) {
          return { regex: g(i.data("validation" + h + "Regex")) };
        },
        validate: function (j, i, h) {
          return (
            (!h.regex.test(i) && !h.negative) || (h.regex.test(i) && h.negative)
          );
        },
      },
      required: {
        ...
        validate: function (j, i, h) {
          return (
            !!(i.length === 0 && !h.negative) || !!(i.length > 0 && h.negative)
          );
        },
        blockSubmit: true,
      },
      match: {
        ...
        init: function (j, h) {
          var i = j
            .parents("form")
            .first()
            .find('[name="' + j.data("validation" + h + "Match") + '"]')
            .first();
          i.bind("validation.validation", function () {
            j.trigger("change.validation", { submitting: true });
          });
          return { element: i };
        },
        validate: function (j, i, h) {
      ...
      max: {
        name: "max",
        init: function (i, h) {
          return { max: i.data("validation" + h + "Max") };
        },
        validate: function (j, i, h) {
          return (
            (parseFloat(i, 10) > parseFloat(h.max, 10) && !h.negative) ||
            (parseFloat(i, 10) <= parseFloat(h.max, 10) && h.negative)
          );
        },
      },
      min: {
        name: "min",
        init: function (i, h) {
          return { min: i.data("validation" + h + "Min") };
        },
        validate: function (j, i, h) {
          return (
      ...
      maxlength: {
        name: "maxlength",
        init: function (i, h) {
          return { maxlength: i.data("validation" + h + "Maxlength") };
        },
        validate: function (j, i, h) {
          return (
      ...
      minlength: {
        name: "minlength",
        init: function (i, h) {
          return { minlength: i.data("validation" + h + "Minlength") };
        },
        validate: function (j, i, h) {
          return (
      ...
      maxchecked: {
        ...
        init: function (j, h) {
          var i = j
            .parents("form")
            .first()
            .find('[name="' + j.attr("name") + '"]');
          i.bind("click.validation", function () {
            j.trigger("change.validation", { includeEmpty: true });
          });
          return {
        ...
        validate: function (j, i, h) {
          return (
            (h.elements.filter(":checked").length > h.maxchecked &&
              !h.negative) ||
      ...
      minchecked: {
        ...
        validate: function (j, i, h) {
          return (
            (h.elements.filter(":checked").length < h.minchecked &&
              !h.negative) ||
  ...
  e.fn.jqBootstrapValidation = function (h) {
    if (f.methods[h]) {
      ...
      if (typeof h === "object" || !h) {
        ...
        e.error(
          "Method " + h + " does not exist on jQuery.jqBootstrapValidation"
        );
        return null;
      }
    }
